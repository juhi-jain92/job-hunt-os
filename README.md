...
|---|---|---|---|
| Discover roles (JSearch, Greenhouse, Wellfound, VC boards) | `stages/job_search.py` | Jobs | free |
...
| Match 7+ roles to warm contacts (2 referrers + recruiter per role) | `stages/referral_match.py` | Referrals | free |
...
| The Today view, up to 5 rows per lane | `stages/today.py` | Today | free |
...
├── stages/             the eight pipeline stages, in run order
...
│   └── greenhouse_sources.py / apify_sources.py   job-board fetchers
...
python3 daily.py --from draft      # resume partway: discover|prune|score|match|network|draft|track|today
```
...
| File | What it holds |
|---|---|
| `config/target_companies.json` | Greenhouse boards to watch, hand-picked targets, blocked companies |
